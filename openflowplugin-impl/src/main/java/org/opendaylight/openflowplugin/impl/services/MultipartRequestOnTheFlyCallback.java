...
final class MultipartRequestOnTheFlyCallback extends AbstractRequestCallback<List<MultipartReply>> {
    ...
    public void onSuccess(final OfHeader result) {
        ...
        if (!(result instanceof MultipartReply)) {
            ...
            // FIX: detected at telstra(v_20161211_090305):
            // there is no guarantee that this code will be code sequentially whe the switch return multiple
            // part message. There force, it is necessary to synchronize and ensure that we first delete
